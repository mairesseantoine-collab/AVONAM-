"""Journal d'audit générique — append-only, chaîné par hash.

Utilisé par `bank/` (audit des flux PSD2) et par `broker/` (audit des
ordres envoyés à un exchange/courtier) : la mécanique est identique dans
les deux cas, seuls les types d'événements journalisés diffèrent.
...
vont typiquement au-delà de 5 ans pour les services de paiement (vérifiez
l'exigence applicable auprès de la BNB/EBA) ; ce module ne purge jamais
automatiquement d'anciennes entrées.
"""
